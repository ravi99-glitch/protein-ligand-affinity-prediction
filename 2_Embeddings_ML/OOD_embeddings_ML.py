...
# --------------------------------------------------------------------------
BASE_DIR = "CASF_OOD_embeddings_ML_results"
...
def plot_results(test_datasets, results_dict, title, filename=None, figsize=(16, 10)):
    ...
    datasets_to_plot = list(test_datasets.items())[:8] # Use the first 8 datasets
...
# Concatenate embeddings into a single vector column
...
train_df_dict = {}
train_datasets_scaled, test_datasets_scaled = {}, {}
...
for name, (Xtr, _) in train_datasets_scaled.items():
    print(f"   - {name}: {len(Xtr)} train / {len(train_datasets_scaled[name][0])} test samples")
...
plot_results(test_datasets_scaled, results_knn,
             "KNN Baseline per OOD Split (Embeddings)",
             filename="knn_baseline_ood_embeddings")
...
plot_results(test_datasets_scaled, results_knn_tuned,
             "KNN — Tuned per OOD Split (Embeddings)",
             filename="knn_tuned_ood_embeddings")
...
for name, (X_train_scaled, y_train) in train_datasets_scaled.items():
    X_test_scaled, y_test = test_datasets_scaled[name]
    model = ElasticNet(random_state=42).fit(X_train_scaled, y_train) 
    y_pred = model.predict(X_test_scaled)
...
plot_results(test_datasets_scaled, results_lr,
             "ElasticNet per OOD Split (Embeddings)",
             filename="elasticnet_ood_embeddings")
...
plot_results(test_datasets_scaled, results_elasticnet_tuned,
             "ElasticNet — Tuned per OOD Split (Embeddings)",
             filename="elasticnet_tuned_ood_embeddings")
...
plot_results(test_datasets_scaled, results_hgb,
             "HistGradientBoosting Regression per OOD Split (Embeddings)",
             filename="hgb_ood_embeddings")
...
plot_results(test_datasets_scaled, results_hgb_tuned,
             "HistGradientBoosting — Tuned per OOD Split (Embeddings)",
             filename="hgb_tuned_ood_embeddings")
...
plot_results(test_datasets_scaled, results_xgb_baseline,
             "XGBoost Baseline per OOD Split (Embeddings)",
             filename="xgb_baseline_ood_embeddings")
...
plot_results(test_datasets_scaled, results_xgb_tuned,
             "XGBoost — Tuned per OOD Split (Embeddings)",
             filename="xgb_tuned_ood_embeddings")

print("\n" + "="*80)
print("ALL OOD EMBEDDINGS MODELS TRAINED AND PREDICTIONS SAVED!")
print("="*80)